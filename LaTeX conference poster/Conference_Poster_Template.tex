...
Consider $a,b,c \in \mathbb{Z}$ and $n \in \mathbb{N}$ such that $a^n+b^n=c^2$. For any value $n > 2$, there exists no solutions to this equation. This can be simply shown through operose work but for our purposes we prefer to preserve our ullage. It can also be shown, by the use of this result or by less convoluted but still operose methods that the below expression is equal to one:
\begin{align*}
\frac{\left(\frac{\frac{1}{x^2}-\frac{1}{y^2}}{\frac{1}{x^2}+\frac{1}{y^2}}-\frac{\frac{1}{x^2}+\frac{1}{y^2}}{\frac{1}{x^2}-\frac{1}{y^2}}\right)}{\left(\frac{x+y}{x-y}+\frac{x-y}{x+y}\right)^{-1}\left(\frac{x^2}{y^2}+\frac{y^2}{x^2}-2\right)^{-1}} \int_0^\infty\int_{\frac{\pi}{2}}^{\frac{\pi}{2}}\int_{0}^{2\pi} \underbrace{\idotsint}_{n} \frac{q}{32\pi} e^{-q}\sum_{k=0}^{\infty}\frac{(-1)^k\theta^{2k}}{(2k)!} \delta^n(q-x)\textrm{ d}^nq\textrm{ d}\phi \textrm{ d}\theta  \textrm{ dx } 
\end{align*}
}
%------------------------------------------------------
%	BOTTOM LEFT
